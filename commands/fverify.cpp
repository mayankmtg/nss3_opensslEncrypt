...
string getFileContentsDecrypt(string filename, string key_string, string iv_string){
	...
	// cout << sizeof(content.c_str())/sizeof(content.c_str()[0]) << " "<<content.length()<< " "<< content.size()<< endl;
...
int main(int argc, char ** argv){
    ...
    string tempFileName = getCorrectedPath(currDirec, argument);
    string signatureFile = argument + ".sign";
    string signature = getCorrectedPath(currDirec, signatureFile);
    argument = tempFileName;
    if(argument.length() < rootDir.length()){
		...
		if(fileReadAllowed(currUser, currUid, argument)){
            ...
            // cout << file_hash << endl;
            string file_hmac = getFileContentsDecrypt(signature, ownerKeyIv.first, ownerKeyIv.second);
            // cout << file_hmac << endl;
