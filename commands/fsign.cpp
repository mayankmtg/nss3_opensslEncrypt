...
int main(int argc, char ** argv){
    ...
    if(argument.length() < rootDir.length()){
        ...
        cout << "Error: file entered does not exists" << endl;
    }
    else if(fileExists(argument)){
        ...
        if(fileWriteAllowed(currUser, currUid, argument)){
            ...
            appendToFile(signatureFile,file_hash,ownerKeyIv.first,ownerKeyIv.second);
            cout << "Signed"<< endl;
        }
        else{
            cout << "Error: Cannot sign a file with no write access" << endl;
        }
    }
    else{
